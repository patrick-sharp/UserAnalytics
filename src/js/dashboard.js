...
window.onload = async function () {
  document
    ...
    .addEventListener("click", openSettingPanel);
  document
    .getElementById("setting_close_button")
    .addEventListener("click", closeSettingPanel);
  ...
  document.getElementById("entertainment_editor").innerHTML =
    categories["Entertainment"].join(", ");
  document.getElementById("social_editor").innerHTML =
    categories["Social"].join(", ");
  document.getElementById("reading_editor").innerHTML =
    categories["Reading"].join(", ");
  document.getElementById("productivity_editor").innerHTML =
    categories["Productivity"].join(", ");
  document.getElementById("uncategorized_editor").innerHTML =
...
async function generateTimeSheet(status, date = null) {
  ...
  for (var i = 0; i < timesheet_data.length; i++) {
    ...
    if (i === 0) {
      // update mostfrequent Icon
      var icon =
        document.getElementById("right_container").children[0].children[1]
          .firstChild;
      icon.src = img.src;
    }
...
function clearUsageData() {
  ...
  setTimeout(() => {
    document.getElementById("clear_usage_success_text").innerHTML = "";
  }, 2000);
}
...
function saveWhitelist() {
  ...
  setTimeout(() => {
    document.getElementById("whitelist_success_text").innerHTML = "";
  }, 2000);
}
...
function saveCategories() {
  ...
  const editors = [
    "entertainment_editor",
    "social_editor",
    "reading_editor",
    "productivity_editor",
    "uncategorized_editor",
  ];
  ...
  setTimeout(() => {
    document.getElementById("categories_success_text").innerHTML = "";
  }, 2000);
}
...
function generateStatistics(titleString, totalTime, timeDiff, domain) {
  ...
  if (timeDiff != 0) {
    ...
    comparison.innerHTML = (
      formatTimeToMinute(timeDiff) + " compared to last time"
    ).replace(/(\+|\-)\d+(min)/, function (v) {
      return (
        (v.includes("+")
          ? "<span class='comp_plus'>"
          : "<span class='comp_minus'>") +
        v +
        "</span>"
      );
    });
...
async function renderGraph(status, date = null) {
  ...
  const order = Array.from(
    Array(polarLabels.length),
    (_, i) => i + 1
  ).reverse();
  if (charts.length === 2) {
    polarChart = charts.pop();
    polarChart.options.plugins.tooltip.callbacks.label =
      updatePolarChartToolTip;
...
async function retrieveDailyData(date = null) {
  ...
  let left_content = generateStatistics(
    "Total Time",
    totalTimeData[0],
    totalTimeData[1],
    ""
  ...
  let right_content = generateStatistics(
    "Most Frequent",
    mostFrequentTimeData[1],
    mostFrequentTimeData[2],
    mostFrequentTimeData[0]
  );
...
async function retrieveWeeklyData() {
  ...
  let left_content = generateStatistics(
    "Total Time",
    weeklyTotalTimeData[0],
    weeklyTotalTimeData[1],
    ""
  );
  left_container.appendChild(left_content);
  ...
  let right_content = generateStatistics(
    "Most Frequent",
    weeklyMostFrequentTimeData[1],
    weeklyMostFrequentTimeData[2],
    weeklyMostFrequentTimeData[0]
  );
  right_container.appendChild(right_content);
}
